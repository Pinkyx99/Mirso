...
function Band({ maxSpeed = 50, minSpeed = 0 }) {
  ...
  const CARD_URL = 'https://cdn.jsdelivr.net/gh/huntabyte/shadcn-svelte-lanyard@main/src/lib/assets/lanyard/card.glb';
  const LANYARD_STRAP_URL = 'https://cdn.jsdelivr.net/gh/huntabyte/shadcn-svelte-lanyard@main/src/lib/assets/lanyard/lanyard.png';
  const CARD_TEXTURE_URL = 'https://i.imgur.com/LbculUL.png';

  const { nodes, materials } = useGLTF(CARD_URL);
  const lanyardTexture = useTexture(LANYARD_STRAP_URL);
  const cardTexture = useTexture(CARD_TEXTURE_URL);
  ...
  // Fix: Explicitly type the `dragged` state to be either a Vector3 or false to prevent type errors.
  const [dragged, drag] = React.useState<THREE.Vector3 | false>(false);
