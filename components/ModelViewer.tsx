...
const Loader: React.FC<{ placeholderSrc?: string }> = ({ placeholderSrc }) => {
  const { progress, active } = useProgress();
  if (!active && placeholderSrc) return null;
  return (
    <Html center>
      {placeholderSrc ? (
        <img alt="Loading model" src={placeholderSrc} width={128} height={128} style={{ filter: 'blur(8px)', borderRadius: 8 }} />
      ) : (
        `${Math.round(progress)} %`
      )}
    </Html>
...
const ModelInner: React.FC<ModelInnerProps> = ({
  ...
  }, [content, fadeIn, autoFrame, pivot, initPitch, initYaw, camera, onLoaded]);
